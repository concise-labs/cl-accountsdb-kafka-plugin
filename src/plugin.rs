...
impl GeyserPlugin for KafkaPlugin {
    ...
    fn on_load(&mut self, config_file: &str) -> PluginResult<()> {
        ...
        info!("conciselabs: on_load config whitelist {:?}", config.program_whitelist);
    ...
    fn update_account(
        ...
        self.unwrap_mut_filter().update_program_whitelist();
        info!("conciselabs: inside update_account: info.owner: {}", Pubkey::new(info.owner));
        if !self.unwrap_filter().wants_program(info.owner) {
            info!("conciselabs: wants_program returned false");
            return Ok(());
        }

        info!("conciselabs: this log, if printed, confirms that there is an account update that needs to be published");
    ...
    fn notify_transaction(
        ...
        let payload = event.transaction.as_ref().unwrap().message.as_ref().unwrap().message_payload.as_ref().unwrap();
        match payload {
            sanitized_message::MessagePayload::Legacy(legacy_message) => {
                ...
                for account in accounts.iter() {
                    info!("conciselabs: notify_transaction: account_keys: account: {}", Pubkey::new(account));
                }
                info!("conciselabs: notify_transaction: program_id_index: {}", program_index);
                info!("conciselabs: notify_transaction: program: {}", Pubkey::new(&accounts[program_index]));
                
                if !self.unwrap_filter().wants_program(&accounts[program_index]) {
                    return Ok(());
                }
            }
...
impl KafkaPlugin {
    ...
    fn unwrap_mut_filter(&mut self) -> &mut Filter {
        self.filter.as_mut().expect("filter is unavailable")
    }
